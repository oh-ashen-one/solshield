...
} from '\var\task\packages\web/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '\var\task\packages\web/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('\var\task\packages\web')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '\var\task\packages\web',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('\var\task\packages\web\.netlify\dist\run\handlers\server.js')
